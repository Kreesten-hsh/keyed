...
`keyed` is a lightweight local FastAPI authentication library for issuing, verifying, and rate-limiting API keys. All validation happens in-process and against your own PostgreSQL database in sub-5ms with zero external cloud proxies.
...
2. **In-Memory Rate Limiting:** Checks sliding-window counters in memory (<1ms). Returns `429 Too Many Requests` with a `Retry-After` header if exceeded.
